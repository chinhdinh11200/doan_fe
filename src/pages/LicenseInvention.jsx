...
import { useInventionsDelete, useInventionsDetail, useInventionList } from '../hooks/inventions';
...
function inventionList() {
  ...
  const columns = [
    ...
    {
      title: <div className="text-center uppercase">Mã BẰNG SÁNG CHẾ/GIẢI THƯỜNG</div>,
      dataIndex: "code",
      key: "code",
    ...
    {
      title: <div className="text-center uppercase">Tên BẰNG SÁNG CHẾ/GIẢI THƯỜNG</div>,
      dataIndex: "name",
      key: "name",
      sortDirections: ["descend", "ascend", "descend"],
      sorter: () => { },
  ...
  const { data: { data: dataUser = [], total } = {}, isLoading } = useInventionList(tableParams);
  const { mutate, isLoading: isLoadingDelete, isSuccess } = useInventionsDelete();
  ...
  const showDeleteModal = (inventionId) => {
    setOpenDeleteModal(true);
    setInventionIdDelete(inventionId);
  };
              ...
              <div className="mb-4">
                ...
                <div className="flex justify-between flex-row-reverse gap-4">
                  ...
                  <NavLink end to="/add-invention" className="btn bg-indigo-500 hover:bg-indigo-600 text-white">
                    ...
                    <span className="ml-2">
                      Thêm bằng sáng chế/giải thưởng
                    </span>
                  </NavLink>
                </div>
              </div>
