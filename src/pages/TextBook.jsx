...
import { useBooksDelete, useBooksDetail, useBookList } from '../hooks/book';
...
function bookList() {
  ...
  const columns = [
    ...
    {
      title: <div className="text-center uppercase">Mã sách/giáo trình</div>,
      dataIndex: "code",
      key: "code",
    ...
    {
      title: <div className="text-center uppercase">Tên sách/giáo trình</div>,
      dataIndex: "name",
      key: "name",
      sortDirections: ["descend", "ascend", "descend"],
      sorter: () => { },
  ...
  const [bookDetailId, setBookDetailId] = React.useState(0);
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const [bookIdDelete, setBookIdDelete] = useState(null);
  const [openDeleteModal, setOpenDeleteModal] = useState(false);
  const [isLoadingg, setIsLoading] = useState(false);
  const { data: { data: dataUser = [], total } = {}, isLoading } = useBookList(tableParams);
  const { mutate, isLoading: isLoadingDelete, isSuccess } = useBooksDelete();
  ...
  return (
    <div className="flex h-screen overflow-hidden">
        ...
        <main>
          <div className="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
            <div className="container max-w-7xl mx-auto mt-3">
              <div className="mb-4">
                <h1 className="font-serif w-fit text-2xl pb-1 mb-4 mx-auto text-center font-bold uppercase border-b border-gray-300">Danh sách Sách, giáo trình</h1>
      ...
      <>
        {showModal ? (
          <ModalDetail setShowModal={setShowModal} bookId={bookDetailId} />
        ) : null}
      </>
    </div>
  );
...
const ModalDetail = ({ bookId, setShowModal }) => {
          ...
          <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full p-6 bg-white outline-none focus:outline-none">
            ...
            <div className="relative border">
              ...
              <div class="flex justify-between py-2 pl-2 border-b">
                <p class="w-1/2 break-all">Mã sách/giáo trình:</p>
                <p class="w-1/2 break-all">{dataBook?.code}</p>
              </div>
              <div class="flex justify-between py-2 pl-2 border-b">
                <p class="w-1/2 break-all">Tên sách/giáo trình:</p>
                <p class="w-1/2 break-all">{dataBook?.name}</p>
              </div>
              <div class="flex justify-between py-2 pl-2 border-b">
                <p class="w-1/2 break-all">Số xuất bản:</p>
                <p class="w-1/2 break-all">{dataBook?.num_decision}</p>
              </div>
              <div class="flex justify-between py-2 pl-2 border-b">
                <p class="w-1/2 break-all">Số tác giả tham gia:</p>
                <p class="w-1/2 break-all">{dataBook?.num_person}</p>
              </div>
              <div class="flex justify-between py-2 pl-2 border-b">
              ...
              <div class="flex justify-between py-2 pl-2 border-b">
                <p class="w-1/2 break-all">Thể loại:</p>
                <p class="w-1/2 break-all">{dataBook?.type}</p>
              </div>
              <div class="flex justify-between py-2 pl-2 border-b">
                <p class="w-1/2 break-all">Số trang:</p>
                <p class="w-1/2 break-all">{dataBook?.num_page}</p>
              </div>
            </div>
          </div>
        </div>
