...
import { useCompilationDelete, useCompilationDetail, useCompilationList } from '../hooks/compilation';
...
function compilationList() {
  ...
  const columns = [
    ...
    {
      title: <div className="text-center">Mã sách/giáo trình</div>,
      dataIndex: "code",
      key: "code",
    ...
    {
      title: <div className="text-center">Tên sách/giáo trình</div>,
      dataIndex: "name",
      key: "name",
      sortDirections: ["descend", "ascend", "descend"],
      sorter: () => { },
        ...
        return (
          <Space size="middle" className="flex justify-center">
            <NavLink
              ...
              to={`/edit-compilation?id=${record.id}`}
              className={({ isActive }) =>
  ...
  const { data: { data: dataUser = [], total } = {}, isLoading } = useCompilationList(tableParams);
  const { mutate, isLoading: isLoadingDelete, isSuccess } = useCompilationDelete();
  ...
  const showDeleteModal = (compilationId) => {
    setOpenDeleteModal(true);
    setCompilationIdDelete(compilationId);
  };
        ...
        <main>
          <div className="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
            <div className="container max-w-7xl mx-auto mt-3">
              <div className="mb-4">
                <h1 className="font-serif w-fit text-2xl pb-1 mb-8 mx-auto text-center font-bold uppercase border-b border-gray-300">Danh sách biên soạn giáo trình bài giảng</h1>
          ...
          <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full p-6 bg-white outline-none focus:outline-none">
            ...
            <div className="relative border">
              ...
              <div className="flex justify-between py-2 pl-2">
                ...
                <p className="w-1/2 break-words">{dataCompialtion?.year.name}</p>
              </div>
            </div>
          </div>
        </div>
